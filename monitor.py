...
class Monitor:
        ...
        for activities in self._activities[DRIVER].values():
            ...
            if len(activities) >= 2:
                for i in range(len(activities) - 1):
                    # Request -> Pickup (Cancellation) -> {Repeat}
                    # Request -> Pickup -> Dropoff -> {Repeat}
    ...
    def _average_ride_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            count += 1
            if len(activities) >= 2:
                total_distance += self._average_ride_distance_helper(activities)
        return total_distance / count

    def _average_ride_distance_helper(self, activity: List[Activity]) -> float:
        """Return the distance drivers have driven on rides.

        """
        curr_dist = 0
        for i in range(len(activity) - 1):
            if activity[i].description == PICKUP:
                if activity[i + 1].description == DROPOFF:
                    dist = manhattan_distance(activity[i].location,
